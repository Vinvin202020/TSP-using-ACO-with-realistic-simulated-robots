...
#define NUM_SAMPLES		100 //how many times we want to compute d_ij before we average it.

//ACS parameter
#define NUM_ITER		8	//how many iteration of ACS to perform
...
void save_pheromone_history(double pher_table_hist[NUM_ITER][MAX_PATROLS][MAX_PATROLS], int n_patrols) {
    ...
    for (int iter = 0; iter < NUM_ITER; ++iter) {
        fprintf(file, "Iteration %d\n", iter + 1); // Add a header for each iteration
        for (int i = 0; i < n_patrols; ++i) {
            for (int j = 0; j < n_patrols; ++j) {
...
int main() 
{
	...
	double pher_table_hist[NUM_ITER][MAX_PATROLS][MAX_PATROLS];
	...
	for (int i= 0; i < NUM_ITER; ++i){
		...
		for (int j = 0; j < n_patrols; j++) {
                          for (int k = 0; k < n_patrols; k++) {
                            pher_table_hist[i][j][k] = pher_table[j * n_patrols + k];
                          }
                      }
	}
	...
	save_pheromone_history(pher_table_hist, n_patrols);
