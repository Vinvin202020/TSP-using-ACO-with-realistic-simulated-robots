...
#define XI           0.4        // local pheromone update parameter ξ
...
// Saves the pheromone history to a CSV file (Run only by Robot 0)
static void save_pheromone_history_csv(void) {
  ...
  // Maps to supervisor loop: for (int iter = 0; iter <= NUM_ITER; ++iter)
...
static void init_pheromone_table(void) {
  ...
  for (int i = 0; i < N_PATROLS; ++i) {
    for (int j = 0; j < N_PATROLS; ++j) {
      ...
      // Initialize History[0] with initial values
  ...
  while (wb_receiver_get_queue_length(receiver) > 0) {
    ...
    if (msg[0] == 'R') {
      ...
      if (n == 5 && sender != my_id) {
        ...
        if (dist <= COMM_RANGE) {
          //printf("[R%d] COMM with R%d at dist=%.3f (sender_iter=%d)\n",
          //       my_id, sender, dist, sender_iter);

          const char *p = msg + consumed;
          int iters_to_read = (nIters < N_ITERS) ? nIters : N_ITERS;
          int start_index = (current_iter > 0) ? (current_iter - 1) : 0;

          // Only update array entries; pheromone update happens ONLY at iteration end
...
static void go_to_patrol(double *vL, double *vR) {
  ...
  *vR += fwd + omega;
  ...
  if (dist < ARRIVE_EPS) {
    ...
    if (VERBOSE_BOTS) {
      printf("[R%d] Arrived at patrol %d\n", my_id, target_patrol);
    }
  }
}
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    if (!has_target) {
      ...
      if (tour_closed) {
        ...
        // --- SNAPSHOT PHEROMONES FOR CSV ---
        // We save the state at the end of the iteration into history index [iter+1]
        // (Index 0 is the initial state, Index 1 is after iter 0, etc.)
