...
static int choose_next_patrol(void) {
  ...
  // compute desirability denominator using D_INV
  double denom = 0.0;
  for (int j = 0; j < N_PATROLS; ++j) {
...
double brute_force_5_nodes_solution(int n_patrols, int* best_path)
{
...
int main(void) {
  ...
  if (my_id == 0) {
    optimal_length = brute_force_5_nodes_solution(N_PATROLS, optimal_path);
